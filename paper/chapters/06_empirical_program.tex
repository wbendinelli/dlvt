...
%
% % citations to add (instruments/methods described in text WITHOUT citation
% % because no key exists in references.bib yet; add before submission):
% %   - Podsakoff et al. (2003/2012), common-method bias and procedural remedies
% %   - Vandenberg & Lance (2000), longitudinal measurement invariance
% %   - Lindell & Whitney (2001), marker-variable technique
% %   - Reise (2012), bifactor modeling
% %   - Nesselroade (1991) / Sliwinski (2008), measurement-burst designs
% %   - Bell (1953) / Duncan et al., accelerated longitudinal designs
% %   - Beal (2015), experience-sampling methods in organizational research
% %   - Voelkle et al. (2012) / Driver, Oud & Voelkle (2017), continuous-time
% %     SEM and the ctsem package
% %   - Carpenter et al. (2017), Stan
% %   - Lakens (2017), equivalence testing / TOST and SESOI justification
% %   - Rizopoulos (2012), joint longitudinal--survival (shared-parameter) models
% %   - Hahn et al. (2011), randomized recovery-training intervention
% %   - a multi-source (360-degree) leadership-assessment methods reference
% ============================================================================
...
supervisor, peer, and subordinate panels; objective unit performance
(unit P\&L or KPI attainment, unit engagement)
...
advance, to a rival measurement model: a bifactor CFA with a general
capital factor plus status and competence group factors, estimated at every
wave. If the general factor is weak (e.g., explained common variance below
...
measurement invariance}: configural, metric, and scalar invariance across
waves must be established (or partial invariance explicitly modeled) before
latent trajectories are interpreted as state dynamics; without scalar
invariance, an apparent damped spiral can be manufactured by drifting item
intercepts. Second, \emph{common-method-bias defenses}: the design separates
sources ($V$ self-reported; $I$ other-reported and archival; $C$ and the
structural half of $O$ archival), separates measurement occasions (ESM
bursts versus quarterly waves), and includes a theoretically neutral marker
variable in every wave so that method variance can be partialled in
sensitivity analyses. Because the central predictions are
...
$1/\mu=5$---a separation of roughly $15\times$. Under the illustrative
...
can resolve both ends of a $15\times$ timescale separation economically. The
design therefore has two interlocking clocks:

\begin{itemize}
\item \emph{Daily ESM bursts for the fast $V$ dynamics.} Two working weeks
...
years, the panel is \emph{accelerated}: participants are enrolled in
staggered role-tenure cohorts (roughly $0$--$2$, $3$--$5$, $6$--$10$, and
...
structural equation modeling (ctsem-style Kalman filtering) or a Bayesian
state-space model (Stan) whose drift is the \emph{exact} DLVT right-hand
side of \eqref{eq:dVdt}--\eqref{eq:complexity}, with additive diffusion and
...
scheduling \citep{sonnentag2007recovery,davidson2010sabbatical}.
...
tests \emph{equivalence} of the $\beta\downarrow$-only arm to control at
...
$V$ trajectory (a shared-parameter structure). This turns a threat into a
